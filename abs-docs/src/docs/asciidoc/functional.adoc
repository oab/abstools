...
done via a special function body of the form `builtin`, with optional
...
body with three arguments: a literal `sqlite3`, the name of the
database file, and the query as a SQL string.  The return value is a
list of ABS values.  The return type of such a function can be:
...
NOTE: When the database file (the second argument to the `builtin`
expression) does not contain a path, the model will look for it in the
`priv` directory, typically
`gen/erl/absmodel/_build/default/lib/absmodel/priv/`.

NOTE: The SQL query currently cannot contain parameters; this will be
